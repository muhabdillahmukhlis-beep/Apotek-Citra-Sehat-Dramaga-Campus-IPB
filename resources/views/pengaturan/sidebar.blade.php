<div class="bg-white p-4 rounded-[24px] border border-[#D4E8D4] shadow-sm space-y-1">
    ...
    {{-- Menu Profil Saya (Bisa diakses Semua Role) --}}
    ...
    {{-- Pembatasan Akses: Hanya Admin yang bisa melihat & mengklik menu sistem lainnya --}}
    @if(auth()->user()->role === 'admin')
        {{-- Info Apotek --}}
        <a href="{{ route('pengaturan.sistem.index') }}" 
           class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('pengaturan.sistem.index') ? 'bg-[#E8F5E9] text-[#2E7D32] font-bold' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-700 font-semibold' }} rounded-xl text-xs transition-all">
            <span class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('pengaturan.sistem.index') ? 'bg-[#2E7D32]' : 'bg-transparent' }}"></span>
    ...
    @else
        {{-- Jika Kasir/Apoteker login, tampilkan menu di bawah ini sebagai teks mati (Disabled) --}}
        <div class="pt-2 mt-2 border-t border-gray-100 space-y-1 opacity-60 select-none">
            <div class="flex items-center gap-3 px-4 py-2.5 text-gray-400 text-xs font-medium"><i class="fas fa-shield-alt w-4 text-center"></i> Keamanan</div>
            <div class="flex items-center gap-3 px-4 py-2.5 text-gray-400 text-xs font-medium"><i class="fas fa-print w-4 text-center"></i> Format Struk</div>
            <div class="flex items-center gap-3 px-4 py-2.5 text-gray-400 text-xs font-medium"><i class="fas fa-cloud-upload-alt w-4 text-center"></i> Backup & Restore</div>
            <div class="flex items-center gap-3 px-4 py-2.5 text-gray-400 text-xs font-medium"><i class="fas fa-history w-4 text-center"></i> Log Audit</div>
            <div class="flex items-center gap-3 px-4 py-2.5 text-gray-400 text-xs font-medium"><i class="fas fa-bell w-4 text-center"></i> Notifikasi</div>
        </div>
    @endif
</div>